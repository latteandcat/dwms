...
public class Admin_Controller extends Controller{
	 ...
	 */
	public void index(){
		render("/d_admin/admin_main.html");
	}
	//药品到期提醒
	public void  outofDatelist(){
		...
		//获得一周后的时间与药品的失效日期作比较获得这一周内即将过期的药品
	 ...
	 */
	public void drug(){
		render("/d_admin/admin_drug.html");
	}
	//处方药
	public void cflist(){
	...
	public void inventory(){
		render("/d_admin/admin_inventory.html");
	}
	//库存列表的初始化
	...
	public void supplier(){
		render("/d_admin/admin_supplier.html");
	}
	//供应商列表的初始化
	 ...
	 */
	public void inout(){
		render("/d_admin/admin_inout.html");
	}
	//入库记录列表的初始化
	public void inlist(){
	 ...
	 */
	public void user(){
		render("/d_admin/admin_user.html");
	}
	//查找用户
	public void userlist(){
		...
		int pageNumber = Integer.parseInt(array[2]) ;
		int infNumber = Integer.parseInt(array[3]);
		Page<dwms_user> users = dwms_user.dao.paginate(pageNumber, infNumber, "SELECT *", 
				"FROM dwms_user where username like '%"+array[0]
						+"%' and poisition like '%"+array[1]+"%'");
		renderJson(users);
	}
	//修改用户信息
	public void updateUser(){
		String[] array = getParaValues("upArray[]");
		int id = Integer.parseInt(array[0]);
		dwms_user user = dwms_user.dao.findById(id);
		user.set("id", id);
		user.set("username",array[1]);
		user.set("password",array[2]);
		user.set("name",array[3]);
		user.set("poisition",array[4]);
		user.update();
		renderNull();
	}
	...
	public void deleteUser() { 
		dwms_user.dao.deleteById(getPara("id"));
		renderText("success");
	}
	//添加用户
	public void addUser() throws NoSuchAlgorithmException, UnsupportedEncodingException{
			String[] array = getParaValues("uArray[]");
			String name = array[0];
			String pw=array[1];
			String password=EncoderByMd5(array[1]);
			System.out.println(password);
			List<dwms_user> auser = dwms_user.dao.find("select * from dwms_user where username = '"+ name +"' ");
			if(auser.isEmpty()){
				dwms_user user=getModel(dwms_user.class,"user");
				List<dwms_user> users = dwms_user.dao.find("select * from dwms_user where id in(select max(id) from dwms_user)");
				if(users.size()==0){
					user.set("id", 1);
					user.set("username",array[0]);
					user.set("password",password);
					user.set("name",array[2]);
					user.set("poisition",array[3]);
				}else{
					user.set("id", users.get(0).getInt("id")+1);
					user.set("username",array[0]);
					user.set("password",password);
					user.set("name",array[2]);
					user.set("poisition",array[3]);
	 ...
	 */
	public void log(){
		render("/d_admin/admin_log.html");
	}
	//查找日志信息
	public void loglist(){
		String[] array = getParaValues("larray[]");
		if(array[3]==""){
			array[3]="9999-9-9";//如果查询的终止时间没有传值的话，使日期最大化
		}else{
			array[3]+=" 23:59:59";//加上时间最大，包括终止时间的那一天
		...
		int infNumber = Integer.parseInt(array[5]);
		Page<dwms_log> logs;
		if(array[1].equals("operation_user")){
			logs= dwms_log.dao.paginate(pageNumber, infNumber, "SELECT *"
					,"FROM dwms_log where "+"operation_time>='"+array[2]+
						"' and operation_time <='"+array[3]+"' and operation_user like '%"+array[0]+"%'");
		}else{
			logs= dwms_log.dao.paginate(pageNumber, infNumber, "SELECT *"
					,"FROM dwms_log where operation_genre like '%"+array[0]+"%' and "+
						"operation_time>='"+array[2]+"' and operation_time<='"+array[3]+"'");
		}
		renderJson(logs);
	}
	//删除日志
	public void deleteLog(){
		dwms_log.dao.deleteById(getPara("id"));
		renderText("success");
	}
	//清空日志
	public void deleteAll(){
		Db.update("truncate table dwms_log");
		renderText("success");
	}
	/*
	 * ************************************导入EXCEL************************************
